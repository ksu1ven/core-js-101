...
function isTriangle(a, b, c) {
  ...
  if (c ** 2 === a ** 2 + b ** 2 || a ** 2 === c ** 2 + b ** 2
    || b ** 2 === a ** 2 + c ** 2) return true;
  return false;
}
...
function isInsideCircle(circle, point) {
  if (point.x - circle.center.x >= circle.radius) return false;
  if (point.y - circle.center.y >= circle.radius) return false;
  if (circle.radius < Math.sqrt((point.y - circle.center.y) ** 2
  + (point.x - circle.center.x) ** 2)) return false;
  return true;
}
...
function findFirstSingleChar(str) {
  ...
  for (let i = 0; i < arr.length; i += 1) {
    const filtered = arr.filter((el) => el === arr[i]);
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i < splitPathes[0].length; i += 1) {
    const foundValue = splitPathes[0][i];
    const filtered = splitPathes.filter((el) => el[i] === foundValue);
    if (filtered.length === splitPathes.length) result.push(foundValue);
  }
